...
today = date.today() # To get Today's Date. db=mysql.connector.connect(host="sql6.freesqldatabase.com",user="sql6582218",passwd="KNAZUQReNL",database="sql6582218")
cursor=db.cursor()
...
M.sendWTPM(Name[i][j],Number[i]) #This will send Whatsapp Messagetothe person
...
Text = Text.read() #Read or stored content of Holiday[i] toText
...
from email.message import EmailMessage # It provides creating or modifying
structured messages.
...
msg = EmailMessage()
Sender_Mail = "hbd.service.01@gmail.com" password = "hfsuwibmkiqtdmvm" def sendMail(Name,Reciver_Mail):
server = smtplib.SMTP('smtp.gmail.com',587) #Created gmail's server, and
connected to gmail API
# Adding transfer layered security
server.starttls()
server.login(Sender_Mail,password) # Email, App password are inserted. msg['Subject'] = 'Testing of HBD Service!' msg['From'] = Sender_Mail
msg['To'] = Reciver_Mail
msg.set_content(f'Happy birthday {Name}!\nI hope all your birthday wishes anddreams come true.')
...
img = 'B1.jpg' body = f'Happy birthday {Name}!\nI hope all your birthday wishes and dreams
come true.' pywhatkit.sendwhats_image(Number,img,body,15,False,3)
#FM: Festival Message
...
# Adding transfer layered security
server.starttls()
server.login(Sender_Mail,password) # Email, App password are inserted. msg['Subject'] = 'Testing of Festival Service!' msg['From'] = Sender_Mail
msg['To'] = Reciver_Mail
...
body = f'{Text}' pywhatkit.sendwhats_image(Number,Photo,body,15,False,3)